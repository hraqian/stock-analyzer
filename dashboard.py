...
# ---------------------------------------------------------------------------
# Contextual help tooltip builder
# ---------------------------------------------------------------------------

def _help_text(terms: dict[str, str]) -> str:
    """Format a ``{term: explanation}`` dict into a single help-tooltip string.

    Returns a newline-separated markdown string suitable for the ``help=``
    parameter on ``st.subheader``, ``st.expander``, etc.
    """
    return "\n\n".join(f"**{term}** — {explanation}" for term, explanation in terms.items())
...
def render_stock_overview(
    ...
    col1, col2, col3, col4 = st.columns([2, 1, 1, 1])
    with col1:
        st.markdown(f"### {name} ({result.ticker})", help=_overview_help)
        meta_parts = []
        if info.get("sector") and info["sector"] != "N/A":
            meta_parts.append(info["sector"])
        if info.get("exchange") and info["exchange"] != "N/A":
    ...
    st.markdown("---")
    st.subheader("Quick Signal", help=_help_text({
...
def render_backtest_section(
    ...
    # ── Recommendation ────────────────────────────────────────────────────
    st.subheader("Backtested Recommendation", help=_help_text({
        "Signal (BUY/SELL/HOLD)": "The strategy's current recommendation. BUY = open a long position, SELL = close or short, HOLD = stay put.",
        "Confidence": "How strong the signal is. High = multiple factors agree, Medium = mixed signals, Low = marginal call.",
        "Effective Score": "The blended indicator + pattern score (0-10) that the strategy used to make this call.",
    }))
    ...
    # ── Performance ───────────────────────────────────────────────────────
    st.subheader("Performance Summary", help=_help_text({
    ...
    # ── Equity curve ──────────────────────────────────────────────────────
    st.subheader("Equity Curve", help=_help_text({
...
def render_dca_section(
    ...
    st.subheader(f"DCA Results — {mode_label}", help=_help_text({
...
def _build_dca_scanner_results_html(
    ...
    header = (
        ...
        "<th title='Current market regime: Bull, Recovery, Sideways, Bear, or Crisis. Sideways and Recovery regimes tend to be better for dip-buying.' style='cursor:help;'>Regime</th>"
...
def _render_watchlist_analysis(signals: list[WatchlistSignal]) -> None:
    ...
    st.subheader("Active Strategy Signals", help=_help_text({
    ...
    if dca_signals:
        st.subheader("DCA Context", help=_help_text({
