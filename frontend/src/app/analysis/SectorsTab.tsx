  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 space-y-5">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          onClick={() => onAnalyzeTicker?.(detail.etf)}
              ...
              {detail.top_movers.map((m) => (
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      onClick={() => onAnalyzeTicker?.(m.ticker)}
...
interface SectorsTabProps {
  /** Called when user wants to analyze a specific ticker (switches to Single Stock tab). */
  onAnalyzeTicker?: (ticker: string) => void;
}

export default function SectorsTab({ onAnalyzeTicker }: SectorsTabProps) {
  const { user } = useAuth();
